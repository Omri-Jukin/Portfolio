...
function TemplateSelector({
  ...
  const { data: templates = [] } = api.intakes.templates.getAll.useQuery();
  ...
  const handleTemplateSelect = (templateId: string) => {
    ...
    if (template) {
      ...
      const templateData = template.templateData as unknown as IntakeFormData;

      // Merge template data with existing form data (preserve pre-filled contact info)
      setFormData((prev) => ({
        ...prev,
        project: {
          ...templateData.project,
          // Keep existing project data if it exists
          title: templateData.project.title || prev.project.title,
          description:
            templateData.project.description || prev.project.description,
        },
        org: templateData.org || prev.org,
        additional: {
          ...prev.additional,
          ...templateData.additional,
        },
      }));
    }
  };
    ...
    >
      <Stack direction="row" spacing={2} alignItems="center">
        <FormControl fullWidth size="small">
          <InputLabel>Select a Template (Optional)</InputLabel>
          <Select
