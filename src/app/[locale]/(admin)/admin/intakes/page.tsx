...
const AdminIntakesList = () => {
  ...
  const deleteIntakeMutation = api.intakes.delete.useMutation({
    onSuccess: () => {
      ...
      setDeleteIntakeDialogOpen(false);
      setDeleteIntakeConfirmDialogOpen(false);
      setIntakeToDelete(null);
      setIntakeIdConfirm("");
      void refetchIntakes();
    },
    onError: (error) => {
      setSnackbarMessage(error.message || "Failed to delete intake");
      setSnackbarOpen(true);
    },
  });
  ...
  const [deleteIntakeDialogOpen, setDeleteIntakeDialogOpen] = useState(false);
  const [deleteIntakeConfirmDialogOpen, setDeleteIntakeConfirmDialogOpen] =
    useState(false);
  const [intakeToDelete, setIntakeToDelete] = useState<{
    id: string;
    email: string;
    name: string;
    createdAt: string | Date | number;
  } | null>(null);
  const [intakeIdConfirm, setIntakeIdConfirm] = useState("");
  ...
  return (
    <ClientOnly skeleton>
      ...
      >
        ...
        {/* Delete Intake - First Verification Dialog */}
        <Dialog
          open={deleteIntakeDialogOpen}
          onClose={() => {
            setDeleteIntakeDialogOpen(false);
            setIntakeToDelete(null);
            setIntakeIdConfirm("");
        ...
        >
          <DialogTitle>Confirm Intake Deletion</DialogTitle>
          <DialogContent>
            <Typography variant="body1" sx={{ mb: 2 }}>
              You are about to delete the following intake:
            </Typography>
            {intakeToDelete && (
              <Box sx={{ mb: 2 }}>
                <Typography variant="body2">
                  <strong>Email:</strong> {intakeToDelete.email}
                </Typography>
                <Typography variant="body2">
                  <strong>Name/Organization:</strong>{" "}
                  {intakeToDelete.name || "Unknown"}
                </Typography>
                <Typography variant="body2">
                  <strong>Created:</strong>{" "}
                  {intakeToDelete.createdAt
                    ? format(
                        new Date(intakeToDelete.createdAt),
                        "MMM dd, yyyy HH:mm"
                      )
                    : "N/A"}
                </Typography>
            ...
            <Typography variant="body2" sx={{ mb: 1 }}>
              To confirm, please type the intake ID:
            </Typography>
            <Typography
              variant="body2"
              sx={{
            ...
            <TextField
              fullWidth
              label="Intake ID"
              value={intakeIdConfirm}
              onChange={(e) => setIntakeIdConfirm(e.target.value)}
              placeholder="Paste intake ID here"
              error={
                intakeIdConfirm !== "" && intakeIdConfirm !== intakeToDelete?.id
              }
              helperText={
                intakeIdConfirm !== "" && intakeIdConfirm !== intakeToDelete?.id
                  ? "ID does not match"
          ...
          <DialogActions>
            ...
            <Button
              onClick={() => {
                if (intakeIdConfirm === intakeToDelete?.id) {
                  setDeleteIntakeDialogOpen(false);
                  setDeleteIntakeConfirmDialogOpen(true);
                }
              }}
              variant="contained"
              color="warning"
              disabled={
                intakeIdConfirm !== intakeToDelete?.id ||
                deleteIntakeMutation.isPending
              }
            >
              Continue
            </Button>
        ...
        {/* Delete Intake - Final Verification Dialog */}
        <Dialog
          open={deleteIntakeConfirmDialogOpen}
          onClose={() => {
            setDeleteIntakeConfirmDialogOpen(false);
            setIntakeToDelete(null);
            setIntakeIdConfirm("");
        ...
        >
          ...
          <DialogContent>
            ...
            <Typography variant="body1" sx={{ mb: 2 }}>
              You are about to <strong>permanently delete</strong> the intake
              for:
            </Typography>
            {intakeToDelete && (
              <Box
            ...
            <Typography variant="body2" color="error.main">
              This action cannot be undone. All related notes, status history,
              and reminders will also be deleted.
            </Typography>
          </DialogContent>
          <DialogActions>
            <Button
